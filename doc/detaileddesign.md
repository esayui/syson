...
> 版本: 2026.5.0 | 日期: 2026-06-14 | 分支: feature/chinese-localization
...
| # | 项目 | 描述 | 阻塞因素 |
|---|------|------|---------|
| 1 | **MatrixView 表格拖放** | 支持从资源管理器拖入元素到表格，自动添加为行 | Sirius 表格组件原生不支持 DropTool，需研究替代方案 |
| 2 | **MatrixView 表格新建行** | 在表格中直接创建新的 PartUsage 元素 | 需要注册 CreateTool，类似 RTV 的 `addRequirement` |
| 3 | **GanttView 数据渲染** | GanttView 甘特图需要完善的 AQL 表达式 | `startTimeExpression` / `endTimeExpression` 依赖元素实际属性 |
| 4 | **TableView 数据渲染** | TableView 表格同样需要数据填充 | 当前 `getExposedElements()` 可能返回空 |
| 5 | **表格数据为空** | MatrixView 表格使用 `self.ownedElement`，需验证 OV-3 等视图的数据是否正确显示 | AQL 表达式可能需要调整为 `self.ownedMember` 或其他方式 |
...
| 3 | Table/Gantt 创建服务仅在 application 层可用 | 初始化器 | 已通过添加 Maven 依赖解决 |
