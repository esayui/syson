 ...
 */
@Service
public class DodafOperationalNodeStyleProvider implements INodeStyleProvider {

    public static final String NODE_SYSML_IMPORTED_PACKAGE = "customnode:dodafoperationalnode";

    @Override
    public Optional<String> getNodeType(NodeStyleDescription nodeStyle) {
        if (nodeStyle instanceof DodafOperationalNodeStyleDescription) {
    ...
    public Optional<INodeStyle> createNodeStyle(NodeStyleDescription nodeStyle, ILayoutStrategy childrenLayoutStrategy, AQLInterpreter interpreter, VariableManager variableManager) {
        ...
        if (nodeType.isPresent()) {
            return Optional.of(DodafOperationalNodeStyle.newDodafOperationalNodeStyle()
                    .background(Optional.ofNullable(((DodafOperationalNodeStyleDescription) nodeStyle).getBackground())
