...
/**
 * Java services for the DoDAF Matrix View table.
 */
@Configuration
public class DoDAFMatrixViewJavaServiceProvider implements IJavaServiceProvider {

    @Override
    public List<Class<?>> getServiceClasses(View view) {
        var descriptions = view.getDescriptions();
        var optDesc = descriptions.stream()
                .filter(desc -> DoDAFMatrixTableDescriptionProvider.DESCRIPTION_NAME.equals(desc.getName()))
                .findFirst();
