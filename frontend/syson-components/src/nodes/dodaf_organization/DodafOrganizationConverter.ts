...
import { GQLDodafOrganizationStyle, DodafOrganizationData } from './DodafOrganization.types';

const defaultPosition: XYPosition = { x: 0, y: 0 };

const toDodafOrganization = (
  gqlDiagram: GQLDiagram,
  gqlNode: GQLNode<GQLDodafOrganizationStyle>,
  gqlParentNode: GQLNode<GQLNodeStyle> | null,
  nodeDescription: GQLNodeDescription,
  isBorderNode: boolean,
  gqlEdges: GQLEdge[]
): Node<DodafOrganizationData> => {
  const {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
  ...
  const data: DodafOrganizationData = {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
    descriptionId,
  ...
  const node: Node<DodafOrganizationData> = {
    id,
...
export class DodafOrganizationConverter implements INodeConverter {
  canHandle(gqlNode: GQLNode<GQLNodeStyle>) {
    return gqlNode.style.__typename === 'DodafOrganizationStyle';
  }
  ...
  handle(
    ...
    if (nodeDescription) {
      nodes.push(toDodafOrganization(gqlDiagram, gqlNode, parentNode, nodeDescription, isBorderNode, gqlEdges));
    }
