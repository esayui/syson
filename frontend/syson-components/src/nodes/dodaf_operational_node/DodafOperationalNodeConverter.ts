...
import { GQLDodafOperationalNodeStyle, DodafOperationalNodeData } from './DodafOperationalNode.types';

const defaultPosition: XYPosition = { x: 0, y: 0 };

const toDodafOperationalNode = (
  gqlDiagram: GQLDiagram,
  gqlNode: GQLNode<GQLDodafOperationalNodeStyle>,
  gqlParentNode: GQLNode<GQLNodeStyle> | null,
  nodeDescription: GQLNodeDescription,
  isBorderNode: boolean,
  gqlEdges: GQLEdge[]
): Node<DodafOperationalNodeData> => {
  const {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
  ...
  const data: DodafOperationalNodeData = {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
    descriptionId,
  ...
  const node: Node<DodafOperationalNodeData> = {
    id,
...
export class DodafOperationalNodeConverter implements INodeConverter {
  canHandle(gqlNode: GQLNode<GQLNodeStyle>) {
    return gqlNode.style.__typename === 'DodafOperationalNodeStyle';
  }
  ...
  handle(
    ...
    if (nodeDescription) {
      nodes.push(toDodafOperationalNode(gqlDiagram, gqlNode, parentNode, nodeDescription, isBorderNode, gqlEdges));
    }
