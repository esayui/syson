...
import { GQLDodafInformationExchangeStyle, DodafInformationExchangeData } from './DodafInformationExchange.types';

const defaultPosition: XYPosition = { x: 0, y: 0 };

const toDodafInformationExchange = (
  gqlDiagram: GQLDiagram,
  gqlNode: GQLNode<GQLDodafInformationExchangeStyle>,
  gqlParentNode: GQLNode<GQLNodeStyle> | null,
  nodeDescription: GQLNodeDescription,
  isBorderNode: boolean,
  gqlEdges: GQLEdge[]
): Node<DodafInformationExchangeData> => {
  const {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
  ...
  const data: DodafInformationExchangeData = {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
    descriptionId,
  ...
  const node: Node<DodafInformationExchangeData> = {
    id,
...
export class DodafInformationExchangeConverter implements INodeConverter {
  canHandle(gqlNode: GQLNode<GQLNodeStyle>) {
    return gqlNode.style.__typename === 'DodafInformationExchangeStyle';
  }
  ...
  handle(
    ...
    if (nodeDescription) {
      nodes.push(toDodafInformationExchange(gqlDiagram, gqlNode, parentNode, nodeDescription, isBorderNode, gqlEdges));
    }
