...
import { GQLDodafSystemNodeStyle, DodafSystemNodeData } from './DodafSystemNode.types';

const defaultPosition: XYPosition = { x: 0, y: 0 };

const toDodafSystemNode = (
  gqlDiagram: GQLDiagram,
  gqlNode: GQLNode<GQLDodafSystemNodeStyle>,
  gqlParentNode: GQLNode<GQLNodeStyle> | null,
  nodeDescription: GQLNodeDescription,
  isBorderNode: boolean,
  gqlEdges: GQLEdge[]
): Node<DodafSystemNodeData> => {
  const {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
  ...
  const data: DodafSystemNodeData = {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
    descriptionId,
  ...
  const node: Node<DodafSystemNodeData> = {
    id,
...
export class DodafSystemNodeConverter implements INodeConverter {
  canHandle(gqlNode: GQLNode<GQLNodeStyle>) {
    return gqlNode.style.__typename === 'DodafSystemNodeStyle';
  }
  ...
  handle(
    ...
    if (nodeDescription) {
      nodes.push(toDodafSystemNode(gqlDiagram, gqlNode, parentNode, nodeDescription, isBorderNode, gqlEdges));
    }
