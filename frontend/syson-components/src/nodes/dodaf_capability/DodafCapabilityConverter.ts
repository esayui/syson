...
import { GQLDodafCapabilityStyle, DodafCapabilityData } from './DodafCapability.types';

const defaultPosition: XYPosition = { x: 0, y: 0 };

const toDodafCapability = (
  gqlDiagram: GQLDiagram,
  gqlNode: GQLNode<GQLDodafCapabilityStyle>,
  gqlParentNode: GQLNode<GQLNodeStyle> | null,
  nodeDescription: GQLNodeDescription,
  isBorderNode: boolean,
  gqlEdges: GQLEdge[]
): Node<DodafCapabilityData> => {
  const {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
  ...
  const data: DodafCapabilityData = {
    targetObjectId,
    targetObjectLabel,
    targetObjectKind,
    descriptionId,
  ...
  const node: Node<DodafCapabilityData> = {
    id,
...
export class DodafCapabilityConverter implements INodeConverter {
  canHandle(gqlNode: GQLNode<GQLNodeStyle>) {
    return gqlNode.style.__typename === 'DodafCapabilityStyle';
  }
  ...
  handle(
    ...
    if (nodeDescription) {
      nodes.push(toDodafCapability(gqlDiagram, gqlNode, parentNode, nodeDescription, isBorderNode, gqlEdges));
    }
